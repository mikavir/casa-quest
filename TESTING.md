...
| | If user does not meet the required fields, an error message will appear to show that regitration have failed | Tested by making the password different from the confirmation password | Error message appeared that the registration failed | Test concluded and passed | ![gif](documentation/testing/defensive-programming/defensive-programming-register2.gif) |
| Login | | | | | |
